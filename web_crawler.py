...
class web_craw:
	...
	def parse_onepage(self,result,niveau,doc_fr,corpus_fr,doc_en,corpus_en):
		self.stage_titre = ''
		self.stage_period = ''
		self.stage_niveau = ''
		self.stage_branche = ''
		...
		for tag3 in tags3:
			if tag3.span.string == "Partenaire":
				...
				self.stage_adresse = tag3.p.text
				self.stage_adresse.replace(u'^\n','')
				#partenaireObj2 = re.match(r'(.*)<br>(\d*) (.*)<br>(.*)', tag4['p'], re.M)
				# partenaireObj1 = re.match(r'(.*)<br>(.*?) (.*)<br>(.*)', tag3.p.text, re.M)
				# if partenaireObj1:
				#
				# 	self.stage_adresse = re.sub(r'<br>',',',partenaireObj1.group(1))
				# 	self.stage_codePostal = partenaireObj1.group(2)
				# 	self.stage_ville = partenaireObj1.group(3)
				# 	self.stage_pays = partenaireObj1.group(4)
				# 	self.stage_adresse.replace(u'\xa0', u'').encode('utf-8')
				# 	self.stage_codePostal.replace(u'\xa0', u'').encode('utf-8')
				# 	self.stage_ville.replace(u'\xa0', u'').encode('utf-8')
				# 	self.stage_pays.replace(u'\xa0', u'').encode('utf-8')
				#
				# #print(self.stage_pays)
				# else:
				# 	self.stage_adresse = ''
				# 	self.stage_codePostal = ''
				# 	self.stage_ville = ''
		...
		if tag5.span.string == "Sujet de stage":
			...
			if self.niveau == 'PR':
				...
				self.stage_branche = tag5.p.find_next("span").find_next("span").string
				self.stage_filliere = ''
...
def main():
	print('begin!')
	xmlfile_fr = open('fr' + '.xml', 'w',encoding="utf-8")
	...
	xmlfile_en = open('en' + '.xml', 'w',encoding="utf-8")
	...
	for ni in niveaus:
		...
		aaa = int(len(crawer.stage_numero) / 6)
		for i in range(0, len(crawer.stage_numero)):
			...
			try:
				...
				crawer.parse_onepage(result2,ni, doc1_fr, corpus_fr,doc1_en, corpus_en)
			except Exception:
